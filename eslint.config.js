...
const globals_all = {
    ...prep_globals(
        r('src/types/auto-imports.d.ts', 'utf8')
            .match(/const (\w+):/g)
            .map(x => x.slice(6, -1))
            .join(' '),
    ),
...
export default [
    ...
    ...svelte.configs['flat/recommended'],
    {
        ignores: [
            'misc/**/*',
            ...'node_modules,dist,dist-native,android,ios,public,project.inlang,src/lib/paraglide'
                .split(',')
                .map(x => x + '/**/*'),
            'build.js',
        ],
    ...
    {
        files: ['vite.config*.js', 'postcss.config.js', 'deploy_update.js'],
        languageOptions: {
